...
{
		if (DEBUG_THREADS) fprintf(stderr,"%d] Unpausing thread %d of %d\n",PTHREAD_NUM,i,paused_threads_count(s));
...
{
	if (DEBUG_THREADS) fprintf(stderr,"%d]Inside quiesce_threads (total=%d)\n", PTHREAD_NUM, (MAXPRI-1));
	for(uint32_t i = 0; i < MAXPRI ; i++) {
		...
		if (s->threadPaused[i] == 0) {
			if (DEBUG_THREADS) fprintf(stderr, "%d] Telling thread %d of %d to pause.\n", PTHREAD_NUM, i, MAXPRI-1);
			pthread_kill(*(s->realtimeThreads[i]), SIGUSR1);
		}
		else {
			if (DEBUG_THREADS) fprintf(stderr, "%d] Thread %d is not ready(%d), skipping.\n", PTHREAD_NUM, i, s->threadPaused[i]);
		}
	}
	...
	do {
		...
		for(uint32_t i = 0 ; i < MAXPRI ; i++) {
			if (i == PTHREAD_NUM) continue; // skip ourself (we are only called by the GC thread)
			if (DEBUG_THREADS) fprintf(stderr, "quiesce_threads check %d (=%d)\n", i, s->threadPaused[i]);
			if (s->threadPaused[i] == 0) fp = FALSE;
			if (DEBUG_THREADS) fprintf(stderr, "quiesce_threads fp = %d\n", fp);
			ssleep(1, 0); // TODO remove before collecting stats
		}
	} while (fp == FALSE);

	if (DEBUG_THREADS) fprintf(stderr, "%d] all threads are paused\n", PTHREAD_NUM);
}

static void handle_resume_signal(int signum)
{
	if (DEBUG_THREADS) fprintf(stderr, "%d] caught signal(%d). resuming.\n", PTHREAD_NUM, signum);
	stashed->threadPaused[PTHREAD_NUM] = 0; // TODO probably a race, but can't use mutex inside a handler.
        ...
        {
            if (DEBUG_THREADS) fprintf(stderr,"%d] paused_thread_count = %d\n",PTHREAD_NUM,paused_threads_count(stashed));
...
{
	sigset_t signal_set;
	if (DEBUG_THREADS) fprintf(stderr, "%d] caught signal(%d). pausing.\n", PTHREAD_NUM, signum);
	...
	if (DEBUG_THREADS) fprintf(stderr, "%d] back from sigsuspend\n", PTHREAD_NUM);
}
...
void install_signal_handler(GC_state s)
{
	...
	if (DEBUG_THREADS) fprintf(stderr, "installing SIGUSR1 handler\n");
	sigusr1.sa_handler = handle_suspend_signal;
	sigemptyset (&sigusr1.sa_mask);
	sigusr1.sa_flags = 0;
	sigaddset (&sigusr1.sa_mask, SIGUSR2);
	if (sigaction (SIGUSR1, &sigusr1, NULL) == -1) {
		if (DEBUG_THREADS) fprintf(stderr, "failed to install SIGUSR1 handler\n");
		exit(-1);
	}

	if (DEBUG_THREADS) fprintf(stderr, "installing SIGUSR2 handler\n");
