...
#define booltype(t, bt, name)                       \
  ...
  writeString (cTypesSMLFd, ")");                   \
  writeNewline2 (cTypesSMLFd, SMLCOM);                       \
  } while (0)
...
int mainX (__attribute__ ((unused)) int argc,
  ...
  mlTypesHFd = stdout; //fopen_safe ("ml-types.h", "w");
  for (int i = 0; mlTypesHPrefix[i] != NULL; i++) 
    writeStringWithNewline2 (mlTypesHFd, mlTypesHPrefix[i], HCOM);
  for (int i = 0; mlTypesHStd[i] != NULL; i++)
    writeStringWithNewline2 (mlTypesHFd, mlTypesHStd[i], HCOM);
  for (int i = 0; mlTypesHSuffix[i] != NULL; i++)
    writeStringWithNewline2 (mlTypesHFd, mlTypesHSuffix[i], HCOM);

  cTypesHFd = stdout; //fopen_safe ("c-types.h", "w");
  cTypesSMLFd = stdout; // fopen_safe ("c-types.sml", "w");

  for (int i = 0; cTypesHPrefix[i] != NULL; i++)
    writeStringWithNewline2 (cTypesHFd, cTypesHPrefix[i], CCOM);
  for (int i = 0; cTypesSMLPrefix[i] != NULL; i++)
    writeStringWithNewline2 (cTypesSMLFd, cTypesSMLPrefix[i], SMLCOM);

  writeNewline (cTypesHFd);writeNewline (cTypesSMLFd);
  writeStringWithNewline2 (cTypesHFd, "/* C */", CCOM);
  writeStringWithNewline2 (cTypesSMLFd, "(* C *)", SMLCOM);
  ...
  writeNewline2 (cTypesHFd, CCOM);writeNewline2 (cTypesSMLFd, SMLCOM);
  ...
  writeNewline2 (cTypesHFd, CCOM);writeNewline2 (cTypesSMLFd, SMLCOM);
  writeStringWithNewline2 (cTypesHFd, "/* Generic integers */", CCOM);
  writeStringWithNewline2 (cTypesSMLFd, "(* Generic integers *)", SMLCOM);
  ...
  writeNewline (cTypesHFd);writeNewline (cTypesSMLFd);
  writeStringWithNewline2 (cTypesHFd, "/* C99 */", CCOM);
  writeStringWithNewline2 (cTypesSMLFd, "(* C99 *)", SMLCOM);
  ...
  writeNewline2 (cTypesHFd, CCOM);writeNewline2 (cTypesSMLFd, SMLCOM);
  writeStringWithNewline2 (cTypesHFd, "/* from <dirent.h> */", CCOM);
  writeStringWithNewline2 (cTypesSMLFd, "(* from <dirent.h> *)", SMLCOM);
  // ptrtype(DIR*, "DirP");
  systype(DIR*, "Word", "DirP");

  writeNewline2 (cTypesHFd, CCOM);writeNewline2(cTypesSMLFd, SMLCOM);
  writeStringWithNewline2 (cTypesHFd, "/* from <poll.h> */", CCOM);
  writeStringWithNewline2 (cTypesSMLFd, "(* from <poll.h> *)", SMLCOM);
  chksystype(nfds_t, "NFds");

  writeNewline2 (cTypesHFd, CCOM);writeNewline2 (cTypesSMLFd, SMLCOM);
  writeStringWithNewline2 (cTypesHFd, "/* from <resource.h> */", CCOM);
  writeStringWithNewline2 (cTypesSMLFd, "(* from <resource.h> *)", SMLCOM);
  chksystype(rlim_t, "RLim");

  writeNewline2 (cTypesHFd, CCOM);writeNewline2 (cTypesSMLFd, SMLCOM);
  writeStringWithNewline2 (cTypesHFd, "/* from <sys/types.h> */", CCOM);
  writeStringWithNewline2 (cTypesSMLFd, "(* from <sys/types.h> *)", SMLCOM);
  ...
  writeNewline2 (cTypesHFd, CCOM);writeNewline2 (cTypesSMLFd, SMLCOM);
  writeStringWithNewline2 (cTypesHFd, "/* from <sys/socket.h> */", CCOM);
  writeStringWithNewline2 (cTypesSMLFd, "(* from <sys/socket.h> *)", SMLCOM);
  chksystype(socklen_t, "Socklen");

  writeNewline2 (cTypesHFd, CCOM);writeNewline2 (cTypesSMLFd, SMLCOM);
  writeStringWithNewline2 (cTypesHFd, "/* from <termios.h> */", CCOM);
  writeStringWithNewline2 (cTypesSMLFd, "(* from <termios.h> *)", SMLCOM);
  chksystype(cc_t, "CC");
  chksystype(speed_t, "Speed");
  chksystype(tcflag_t, "TCFlag");

  writeNewline2 (cTypesHFd, CCOM);writeNewline2 (cTypesSMLFd, SMLCOM);
  writeStringWithNewline2 (cTypesHFd, "/* from \"gmp.h\" */", CCOM);
  writeStringWithNewline2 (cTypesSMLFd, "(* from \"gmp.h\" *)", SMLCOM);
  chksystype(mp_limb_t, "MPLimb");

  writeNewline2 (cTypesHFd, CCOM);writeNewline2 (cTypesSMLFd, SMLCOM);
  for (int i = 0; cTypesHSuffix[i] != NULL; i++)
    writeStringWithNewline2 (cTypesHFd, cTypesHSuffix[i], CCOM);
  for (int i = 0; cTypesSMLSuffix[i] != NULL; i++)
    writeStringWithNewline2 (cTypesSMLFd, cTypesSMLSuffix[i], SMLCOM);
