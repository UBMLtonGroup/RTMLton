...
fun doit kArraySize =
  let open Array
            ...
            fun allocateNewArray size = 
                let
                    val arr2 = timeme array (size,SOME(array(1,Real.fromInt(10))))
                    fun traverseArray j = 
                            if j < (size)
                            then ((if (j mod 1000) =0 
                                    then update(arr2, j,
                                    SOME(array(1,Real.fromInt(j+1))))
                                    else ());
                                   traverseArray (j+1))
                            else ()
                in
            ...
            fun fillHeap i =
                if i < (kArraySize)
                    then (update (arr, i, SOME(array(1,Real.fromInt(i))));
                          fillHeap (i + 1))
                    else ()
       ...
       (*print(heapstats ()^"\n");*)
        ...
        if (let val n = Int.min (1000, (length(arr) div 2) - 1)
           in Real.!= ( let val v= sub (arr, n) in if Option.isSome v then
             sub (Option.valOf v,0) else 0.0 end, (1.0 / Real.fromInt(n)))
           end)
        then print "Failed\n"
        else print "Success\n"
    end
...
val _ = doit 8669770 (*(Option.valOf(Int.fromString(hd (CommandLine.arguments()))))*)
